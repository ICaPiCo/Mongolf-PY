...
class Coin:
    ...
    def update(self):
        ...
        for coin in self.coins[:]:
            if abs(self.player_x - coin[0]) < 10 and abs(self.player_y - coin[1]) < 10:
                self.coins.remove(coin)
                self.score += 1
                # Add new coin to keep total at 10
                self.coins.append([random.randint(0, 600), random.randint(0, 400)])
    ...
    def draw(self):
        ...
        for x, y in self.coins:
            pyxel.circ(x, y, 5, pyxel.COLOR_YELLOW)
        # Draw player (simple circle)
        pyxel.circ(self.player_x, self.player_y, 10, pyxel.COLOR_RED)
        # Draw UI
